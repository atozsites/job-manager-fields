<?php
/**
 * Plugin Name: AtoZ fields for Job Manager
 * Plugin URI:  https://github.com/atozsites/job-manager-fields
 * Description: An example plugin for adding custom fields to the AtoZ Job Manager submission form.
 * Author:      AtoZSites
...
class AtoZSites_Job_Manager_Fields {
	 ...
	 *
	 * @since Custom fields for AtoZSites Job Manager
	 *
	 * @param void
	...
	private function setup_actions() {
		/**
		 * Filter the default fields that ship with AtoZ Job Manager.
		...
		/**
		 * When AtoZ Job Manager is saving all of the default field data, we need to also
	 ...
	 * AtoZ Job Manager filters the data that passes to the registration template, these are the
	...
	function form_fields( $fields ) {
		$fields[ 'company' ][ 'company_office_name' ] = array(
	...
	function update_job_data( $job_id, $values ) {
		...
		$morale = isset ( $values[ 'company' ][ 'company_office_name' ] ) ? sanitize_text_field( $values[ 'company' ][ 'company_office_name' ] ) : null;

		/** By using an underscore in the meta key name, we can prevent this from being shown in the Custom Fields metabox. */
		if ( $morale )
			update_post_meta( $job_id, '_company_office_name', $morale );
	...
	function job_listing_data_fields( $fields ) {
		 ...
		 */
		$fields[ '_company_office_name' ] = array(
			'label'       => 'Company Name', // The field label
			'placeholder' => 'Atoz, etc',     // The default value when adding via backend.
...
add_action( 'init', array( 'AtoZSites_Job_Manager_Fields', 'instance' ) );